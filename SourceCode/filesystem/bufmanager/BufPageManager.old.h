...
public:
	...
	BufType fetchPage(int typeID, int pageID, int& index) {
	    std::cout << "BufPageManager.fetchPage() typeID = " << typeID << " pageID = " << pageID << std::endl;
		BufType b;
		index = replace->find();
		b = addr[index];
		std::cout << "    index = " << index << ", b = " << (int) b << std::endl;
		if (b == NULL) {
			b = allocMem();
			addr[index] = b;
		} else {
...
public:
	...
	BufType getPage(int fileID, int pageID, int& index) {
		...
		std::cout << "BufPageManager.getPage() hash(" << fileID << "," << pageID << ")=" << index << std::endl;
		if (index != -1) {
			access(index);
			return addr[index];
		} else {
